...
<body
            ...
            <ul class="flex flex-col pl-0 mb-0">
                ...
                <li class="mt-0.5 w-full">
                    <a class="py-2.7 dark:text-white dark:opacity-80 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap rounded-lg px-4 font-semibold text-slate-700 transition-colors"
                        ...
                        <div
                            class="mr-2 flex h-8 w-8 items-center justify-center rounded-lg bg-center stroke-0 text-center xl:p-2.5">
                            <i class="relative top-0 text-sm leading-normal text-red-600 ni ni-box-2"></i>
                        </div>
                ...
                <li class="mt-0.5 w-full">
                    <a class="dark:text-white dark:opacity-80 py-2.7 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap px-4 transition-colors"
                        ...
                        <span class="ml-1 duration-300 opacity-100 pointer-events-none ease">Product Sale</span>
                    </a>
                </li>
                ...
                <li class="mt-0.5 w-full">
                    <a class="bg-blue-500/13 dark:text-white dark:opacity-80 py-2.7 text-sm ease-nav-brand my-0 mx-2 flex items-center whitespace-nowrap px-4 transition-colors"
                        ...
                        <span class="ml-1 duration-300 opacity-100 pointer-events-none ease">Cart Items</span>
                    </a>
                </li>
        ...
        <div class="w-full px-6 py-6 mx-auto">
            ...
            <div class="flex flex-wrap -mx-3">
                ...
                <div class="w-full max-w-full px-3 mt-6 md:w-8/12 md:flex-none">
                    ...
                    <form method="POST" action="{{ route('saler.checkouts.store') }}"
                        class="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
                        @csrf
                        <div class="p-6 px-4 pb-0 mb-0 border-b-0 rounded-t-2xl flex justify-between">
                            <h6 class="mb-0 dark:text-white">Selected Items Billing Amount: ₹ <span
                                    id="totalAmount"></span></h6>
                            <button type="submit"
                                                    ...
                                                    <div class="flex mb-2">
                                                        ...
                                                        <div class="mr-2">
                                                            ...
                                                            <input type="number" readonly id="" value="{{ $rate }}"
                                                                class="quantity-input w-full px-3 py-2 border rounded-lg dark:bg-slate-850 dark:text-white text-sm"
                                                                data-rate="" data-target=""
                                                                name="products[{{ $productId }}][customer_product_rate]">
                                                        </div>
                                                        <!-- it show profit percentage (readonly) calculated from final price -->
                                                        <div>
                                                            <label class="text-sm text-green-600 dark:text-white">Profit %:</label>
                                                            <input type="number" readonly
                                                                name="products[{{ $productId }}][customer_profit_percentage]"
                                                                id="profit-percentage-{{ $targetId }}"
                                                                class="profit-percentage-output w-full px-3 py-2 border text-green-600 rounded-lg dark:bg-slate-850 dark:text-white text-sm">
                                                        </div>
                                                    </div>
                                                    <!-- it allow input quantity by default 1 -->
                                                    <input type="number" placeholder="Enter Quantity"
                                                        name="products[{{ $productId }}][customer_purchase_quantity]"
                                                        ...
                                                        max="{{ $product->purchase_unit }}" required>
                                                    <small>Available Quantity: {{ $product->purchase_unit }}</small>
                                                    <br>
                                                    <!-- it allow input final selling price -->
                                                    <input type="number" placeholder="Enter Final Price"
                                                        name="products[{{ $productId }}][customer_product_selling_price]"
                                                        class="selling-price-input w-full px-3 py-2 border rounded-lg dark:bg-slate-850 dark:text-white text-sm"
                                                        data-rate="{{ $rate }}" data-target="{{ $targetId }}"
                                                        id="selling-price-{{ $targetId }}">
                                                </div>
                                            </li>
                                        @endforeach
                                    @endforeach
                        ...
                        <div style="display: flex; justify-content: space-between;">
                            ...
                            @if($hasCheckout)
                                <form action="{{ route('saler.checkout.delete') }}" method="POST" style="padding: 15px;">
                                    ...
                                    @method('DELETE')
                                    <button
                                        class="px-8 py-2 font-bold text-white rounded-lg shadow-md text-xs hover:shadow-xs hover:-translate-y-px active:opacity-85 w-full"
                                        style="background-color: red;" type="submit">Delete Checkout Items</button>
                                </form>
                            @endif
                            ...
                            <div class="flex-auto p-4">
                                <ul class="flex flex-col pl-0 mb-0 rounded-lg">
                                    ...
                                    @foreach($decodedProducts as $productData)
                                        ...
                                        <li
                                            ...
                                            <div class="flex">
                                                <div class="inline-block w-8 h-8 mr-4 text-center">
                                                ...
                                                <div class="flex flex-wrap ">
                                                    ...
                                                    <h6 class="mb-1 text-sm leading-normal text-slate-700 dark:text-white">
                                                        ...
                                                        <span class="font-semibold text-red-600 dark:text-white text-sm"
                                                            style="text-transform: capitalize;">{{ $product?->product_name }}</span>,&nbsp;

                                                        Quantity:
                                                        <span
                                                            class="font-semibold text-red-600 dark:text-white text-sm">{{ $productData['customer_purchase_quantity'] }}</span>,&nbsp;

                                                        Amount:
                                                        <span
                                                            class="font-semibold text-red-600 dark:text-white text-sm">{{ $productData['customer_product_selling_price'] }}</span>,&nbsp;
                                                    ...
                                                    <!-- Hidden Inputs -->
                                                    <input type="hidden"
                                                        name="products[{{ $productData['product_id'] }}][product_id]"
                                                        value="{{ $productData['product_id'] }}">
                                                    <input type="hidden"
                                                        name="products[{{ $productData['product_id'] }}][customer_product_rate]"
                                                        value="{{ $productData['customer_product_rate'] }}">
                                                    <input type="hidden"
                                                        name="products[{{ $productData['product_id'] }}][customer_purchase_quantity]"
                                                        value="{{ $productData['customer_purchase_quantity'] }}">
                                                    <input type="hidden"
                                                        name="products[{{ $productData['product_id'] }}][customer_profit_percentage]"
                                                        value="{{ $productData['customer_profit_percentage'] }}">
                                                    <input type="hidden"
                                                        name="products[{{ $productData['product_id'] }}][customer_product_selling_price]"
                                                        value="{{ $productData['customer_product_selling_price'] }}">
                                    ...
                                    @if($latestCheckout && $latestCheckout->customer_overall_total_amount)
                                        <input type="hidden" name="customer_overall_total_amount"
                                            value="{{ $latestCheckout?->customer_overall_total_amount ?? 0 }}">
                                        <li
                                            class="relative flex justify-between py-2 pr-4 mb-2 border-0 rounded-t-lg rounded-xl text-inherit">
                                            <div class="flex items-center">
                                                <div class="flex">
                                                    <i class="text-black ni ni-box-2 relative top-0.75 text-m"></i> &nbsp;
                                                    &nbsp; &nbsp;
                                                    <h6 class="text-sm leading-normal text-slate-700 dark:text-white">
                                                        Grand Total:
                                                        <!-- it shows customer_overall_total_amount -->
                                                        <span class="font-semibold text-red-600 dark:text-white">₹
                                                            {{ number_format($latestCheckout->customer_overall_total_amount, 2) }}</span>
                                                    </h6>
                                ...
                                <div class="flex-auto p-4">
                                    <div class="flex flex-wrap -mx-3">
                                        <div class="w-full max-w-full px-3 shrink-0 md:w-6/12 md:flex-0">
                                            <div class="mb-4">
                                                <label for="customer_name"
                                                    class="inline-block mb-2 ml-1 font-bold text-xs text-slate-700 dark:text-white/80">Customer's
                                                    Name</label>
                                        ...
                                        <div class="w-full max-w-full px-3 shrink-0 md:w-6/12 md:flex-0">
                                            <div class="mb-4">
                                                <label for="custome_email"
                                                    class="inline-block mb-2 ml-1 font-bold text-xs text-slate-700 dark:text-white/80">Email
                                                    ID</label>
                                                <input type="email" name="custome_email" placeholder="Enter Email ID"
                                                    class="focus:shadow-primary-outline dark:bg-slate-850 dark:text-white text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none" />
                                            </div>
                                        </div>
                                        <div class="w-full max-w-full px-3 shrink-0 md:w-12/12 md:flex-0">
                                            <div class="mb-4">
                                                <label for="custome_mobile"
                                                    class="inline-block mb-2 ml-1 font-bold text-xs text-slate-700 dark:text-white/80">Mobile
                                                    Number</label>
                                                <input type="text" name="custome_mobile"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            // Calculate profit percentage when quantity or selling price changes
            document.querySelectorAll('.quantity-input, .selling-price-input').forEach(input => {
                input.addEventListener('input', function () {
                    ...
                    const sellingPriceInput = document.querySelector(`.selling-price-input[data-target="${target}"]`);
                    const profitPercentageOutput = document.getElementById(`profit-percentage-${target}`);
                    ...
                    if (baseAmount > 0 && sellingPrice > 0) {
                        // Calculate profit percentage: ((Selling Price - Base Amount) / Base Amount) * 100
                        const profitPercentage = ((sellingPrice - baseAmount) / baseAmount) * 100;
                        profitPercentageOutput.value = profitPercentage.toFixed(2);
                    } else {
                        profitPercentageOutput.value = '';
                    }
                });
            });
        });
